...
x0=[5,5]';
fprintf('x0=[%d,%d]\n',x0);
[X, info,perf] = ucminf(@func_basis, x0);
...
fprintf('Global Minimum estimated: [%d,%d] in %d iterations,%d functions evaluated \n',X(:,end),info(4),info(5));
error=sqrt(sum((X'-x_min(3,:)).^2,2));
[p,c]= cvg_rate( error, fignumber);
fignumber=fignumber+1;
fprintf('Convergence rate: %d and constant limit : %d\n',p,c);
make_contour_plot( func, X', x_global_min',fignumber,[] )
fignumber=fignumber+1;
convergence_proof( perf.ng', X', x_min(3,:) ,fignumber )
fignumber=fignumber+1;
%%
...
fprintf('Global Minimum estimated: [%d,%d] in %d iterations,%d functions evaluated \n',X(:,end),info(4),info(5));
error=sqrt(sum((X'-x_min(2,:)).^2,2));
[p,c]= cvg_rate( error, fignumber);
fignumber=fignumber+1;
fprintf('Convergence rate: %d and constant limit : %d\n',p,c);
make_contour_plot( func, X', x_global_min',fignumber,[] )
fignumber=fignumber+1;
convergence_proof( perf.ng', X', x_min(2,:) ,fignumber )
fignumber=fignumber+1;
...
tic
[X, ~,~,OUTPUT] = fminunc(@func_basis, x0,options);
toc
%%
...
[X, ~,~,OUTPUT] = fminunc(@func_basis, x0,options);
toc
fprintf('Global Minimum estimated: [%d,%d] in %d iterations,%d functions evaluated \n',X,OUTPUT.iterations,OUTPUT.funcCount);
%%
